...
//assumes first two elements of input binary file are size of tableau
//assumes first row of input tableau is objective row
...
void store_soln (int m, int n, float A[][n], char* outfile) {
  ...
  fprintf(file_ptr, "%f ",  A[0][n-1]); //objective function value in top RHS corner
...
void simplex(int m, int n, float A[][n], char* vars[]) {
  ...
  while (1) {
    ...
    int pivot_row_idx = -1;
    for (int i = 1; i < m; i++) { //i in range(0, len(A)-1): #iterate thru all rows but do not include last row bc is eqn to max
...
int main(int argc, char *argv[]) {
  ...
  char* append_str = "_out_20240411.txt";
